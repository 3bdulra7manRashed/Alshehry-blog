    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        ...
        <div class="flex items-center gap-3">
            <a href="mailto:{{ $message->email }}?subject=رد: رسالة من موقع {{ config('app.name') }}&body=%0A%0A---%0Aفي {{ $message->created_at->format('Y/m/d') }}، كتب {{ $message->name }}:%0A{{ urlencode($message->message) }}" 
               class="inline-flex items-center px-5 py-2.5 bg-brand-accent text-white font-medium rounded-lg hover:bg-opacity-90 transition-colors">
                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                </svg>
            ...
            @if($message->phone)
                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $message->phone) }}" 
                   target="_blank"
                   class="inline-flex items-center px-5 py-2.5 bg-green-500 text-white font-medium rounded-lg hover:bg-green-600 transition-colors">
